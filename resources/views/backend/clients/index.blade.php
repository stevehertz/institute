...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="row @if(!isset($client)) collapse @endif" id="createCat">
                <div class="col-12">
                    @if(isset($client))
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-4 col-12 form-group mb-0">
                            {!! Form::label('link', trans('labels.backend.clients.fields.link'), ['class' => 'control-label']) !!}
                            {!! Form::text('link', old('link'), ['class' => 'form-control', 'placeholder' => trans('labels.backend.clients.fields.link'), 'required' => false]) !!}
                        </div>

                        <div class="col-lg-4 col-12 form-group mb-0">

                            {!! Form::label('logo',  trans('labels.backend.clients.fields.logo'), ['class' => 'control-label d-block']) !!}
                            @if(isset($client) && ($client->logo != null))
                                {!! Form::file('logo', ['class' => 'form-control w-75 d-inline-block', 'placeholder' => '', 'accept' => 'image/jpeg,image/gif,image/png']) !!}
                                    <img class="d-inline-block" src="{{asset('storage/uploads/'.$client->logo)}}" height="38px">
                            @else
                    ...
                    <div class="table-responsive">
                        ...
                        <table id="myTable"
                            ...
                            <tr>
                                ...
                                <th>@lang('labels.backend.clients.fields.logo')</th>
                                <th>@lang('labels.backend.clients.fields.link')</th>
                                <th>@lang('labels.backend.clients.fields.status')</th>
                                @if( request('show_deleted') == 1 )
...
@push('after-scripts')
    <script>
        ...
        $(document).on('click', '.switch-input', function (e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('admin.clients.status') }}",
                data: {
                    _token:'{{ csrf_token() }}',
                    id: id,
                },
