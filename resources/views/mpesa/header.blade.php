    ...
    <script type="text/javascript">
            ...
            if (!window.loadedSearchData) {
                ...
                var request = $.get('{{ URL::route('get_search_data') }}', function(data) {
                    $('#search').typeahead({
                            ...
                            @if (Auth::check() && $invoice->account->customLabel('client1'))
                        ,{
                            name: 'data',
                            limit: 3,
                            display: 'value',
                            source: searchData(data['{{ $invoice->account->present()->customLabel('client1') }}'], 'tokens'),
                            templates: {
                                header: '&nbsp;<span style="font-weight:600;font-size:16px">{{ $invoice->account->present()->customLabel('client1') }}</span>'
                            }
                        }
                            @endif
                            @if (Auth::check() && $invoice->account->customLabel('client2'))
                        ,{
                            name: 'data',
                            limit: 3,
                            display: 'value',
                            source: searchData(data['{{ $invoice->account->present()->customLabel('client2') }}'], 'tokens'),
                            templates: {
                                header: '&nbsp;<span style="font-weight:600;font-size:16px">{{ $invoice->account->present()->customLabel('client2') }}</span>'
                            }
                        }
                            @endif
                            @if (Auth::check() && $invoice->account->customLabel('invoice_text1'))
                        ,{
                            name: 'data',
                            limit: 3,
                            display: 'value',
                            source: searchData(data['{{ $invoice->account->present()->customLabel('invoice_text1') }}'], 'tokens'),
                            templates: {
                                header: '&nbsp;<span style="font-weight:600;font-size:16px">{{ $invoice->account->present()->customLabel('invoice_text1') }}</span>'
                            }
                        }
                            @endif
                            @if (Auth::check() && $invoice->account->customLabel('invoice_text2'))
                        ,{
                            name: 'data',
                            limit: 3,
                            display: 'value',
                            source: searchData(data['{{ $invoice->account->present()->customLabel('invoice_text2') }}'], 'tokens'),
                            templates: {
                                header: '&nbsp;<span style="font-weight:600;font-size:16px">{{ $invoice->account->present()->customLabel('invoice_text2') }}</span>'
                            }
                        }
                            @endif
        ...
        $(function() {
            ...
            if (isStorageSupported()) {
                @if (Auth::check() && !$invoice->registered)
                localStorage.setItem('guest_key', '{{ $invoice->password }}');
                @endif
            }
        ...
        <div class="navbar-header">
            ...
            <a href="#" id="left-menu-toggle" class="menu-toggle" title="{{ trans('texts.toggle_navigation') }}">
                <div class="navbar-brand">
                    ...
                    <img src="{{ asset('images/small_horizontal_logo.png') }}" width="193" height="25" style="float:left"/>
                </div>
            </a>
        </div>
            ...
            <div class="navbar-form navbar-right">
                ...
                @if (Auth::check())
                    @if (!$invoice->registered)
                        ...
                        @endif
                    @elseif (Utils::isNinjaProd() && (!$invoice->isPro() || $invoice->isTrial()))
                        @if ($invoice->account->company->hasActivePromo())
...
{{--                            @endif--}}
                    ...
                    <ul class="dropdown-menu user-accounts">
                        ...
                        @else
                            @include('user_account', [
                                'account_name' => $invoice->client->name ?: trans('texts.untitled'),
                                'user_name' => $invoice->client ->getDisplayName(),
                                'logo_url' => $invoice->account->getLogoURL(),
                                'selected' => true,
                            ])
                        @endif
                        <li class="divider"></li>
                        @if (Utils::isAdmin() && $invoice->confirmed && Utils::getResllerType() != RESELLER_ACCOUNT_COUNT)
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        @if (Utils::isNinjaProd())
                            @if (Auth::check() && $invoice->hasActivePromo())
                                {!! trans('texts.promotion_footer', [
                                        'link' => '<a href="javascript:showUpgradeModal()">' . trans('texts.click_here') . '</a>'
                                    ]) !!}
                            @elseif (Auth::check() && $invoice->isTrial())
                                {!! trans($invoice->account->getCountTrialDaysLeft() == 0 ? 'texts.trial_footer_last_day' : 'texts.trial_footer', [
                                        'count' => $invoice->account->getCountTrialDaysLeft(),
                                        'link' => '<a href="javascript:showUpgradeModal()">' . trans('texts.click_here') . '</a>'
                                    ]) !!}
                            @endif
                        @else
                            @include('partials.white_label', ['company' => $invoice->account->company])
                        @endif
                    </div>
                </div>
            </div>
