...
@if(Cart::getConditionsByType('coupon') != null)
    @foreach(Cart::getConditionsByType('coupon') as $condition)
        <div class="in-total font-weight-normal  mb-3"> {{ $condition->getValue().' '.$condition->getName()}}
            <span class="font-weight-bold"> {{Session::get('cartCurrency')?Session::get('cartCurrency') : ($cartCurrency['short_code']).' '}}.{{number_format($condition->getCalculatedValue($total),2)}}</span>
...
@if($taxData != null)
    @foreach($taxData as $tax)
        <div class="in-total font-weight-normal  mb-3"> {{ $tax['name']}}
            <span class="font-weight-bold"> {{Session::get('cartCurrency')?Session::get('cartCurrency') : $cartCurrency['short_code'].' '}}.{{number_format($tax['amount'],2)}}</span>
        </div>
    @endforeach
@endif
