...
if (!function_exists('getOrderNumber')) {
    ...
    {
        if (session()->has('uniqueOrderNumber')){
            $orderNumber = session()->get('uniqueOrderNumber');
            return $orderNumber;
        }
        else {
        $orderRand = 'ABCDEFGHJKMNPQRSTUVWXYZ234568922'[rand(0, 30)];
        $orderNumber =  'LM'.$orderRand.Carbon::now()->format('u');
        session()->put('uniqueOrderNumber', $orderNumber);
        return $orderNumber;
        }
    }
}
