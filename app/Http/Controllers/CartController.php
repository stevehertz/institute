...
class CartController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (session()->get('cartCurrency') == null) {
            $cartCurrency = $this->currency;
        } else{
            $cartCurrency  = session()->get('cartCurrency');

        }

        return view($this->path . '.cart.checkout', compact('courses', 'bundles', 'total', 'taxData', 'cartCurrency'));
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        if (!in_array($product->id, $cart_items)) {
            ...
            session()->put('cartCurrency', $request->cartCurrency);
        }
    ...
    public function checkout(Request $request)
    {
        ...
        $total = $courses->sum('price');
        if (session()->get('cartCurrency') == null) {
            $cartCurrency = $request->cartCurrency;
            session()->put('cartCurrency', $request->cartCurrency);
        } else{
            $cartCurrency  = session()->get('cartCurrency');
    ...
    public function offlinePayment(Request $request)
    {
        ...
        $order = $this->makeOrder($request->uniqueOrderId);
    ...
    private function makeOrder($uniqueOrderId)
    {
        ...
        $order = new Order();
        $order->user_id = auth()->user()->id;
        $order->reference_no = $uniqueOrderId;
