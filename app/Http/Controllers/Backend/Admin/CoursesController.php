...
class CoursesController extends Controller
{
    ...
    {
        $enrolments =  DB::table('course_student')->orderBy('course_student.updated_at', 'desc')
            ->join('users', 'course_student.user_id', '=', 'users.id')
    ...
    public function getEnrolmentData(Request $request)
    {
        ...
        return DataTables::of($enrolments)
            ->addIndexColumn()
            ->addColumn('actions', function ($q) use ($request) {
                $view = "";

                $deactivate = view('backend.datatable.action-deactivate')
                    ->with(['route' => route('admin.deactivate.enrolment', ['order' => $q->id])])
                    ->render();
            ...
            ->addColumn('first_name', function ($q) {
