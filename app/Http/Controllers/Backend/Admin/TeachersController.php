        ...
        return DataTables::of($teachers)
            ...
            ->addColumn('actions', function ($q) use ($has_view, $has_edit, $has_delete, $request) {
                ...
                if ($request->show_deleted == 1) {
                    return view('backend.datatable.action-trashed')
                        ->with(['route_label' => 'admin.teachers', 'label' => 'id', 'value' => $q->id]);
                }
            ...
            ->addColumn('status', function ($q) {
                $html = html()->label(html()->checkbox('')->id($q->id)
                    ->checked(($q->active == 1) ? true : false)->class('switch-input')
                    ->attribute('data-id', $q->id)
                    ->value(($q->active == 1) ? 1 : 0) . '<span class="switch-label"></span><span class="switch-handle"></span>')
