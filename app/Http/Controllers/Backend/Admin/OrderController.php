        ...
        foreach ($order->items as $orderItem) {
            ...
            if ($orderItem->item_type == Bundle::class) {
                foreach ($orderItem->item->courses as $course) {
                    ...
                    $associated_exam = Exam::where('course_id', $course->id)->where('status', 1)->first();
                    if ($associated_exam !== null) {
                        $associated_exam->students()->attach($order->user_id);
                    }
                }
            }
            $associated_exam = Exam::where('course_id', $orderItem->item->id)->where('status', 1)->first();
